...
const esquema = z
  .object({
    nomeOrganizacao: z.string().trim().min(2, 'Informe o nome da empresa ou do escritório.').max(120),
  ...
  .refine((dados) => dados.senha === dados.confirmacao, { path: ['confirmacao'], message: 'As senhas não conferem.' });
...
export function CadastroPage() {
  ...
  const formulario = useForm<Dados>({
    ...
    defaultValues: { nomeOrganizacao: '', nome: '', email: '', senha: '', confirmacao: '', plano: planoDaUrl, termos: false as unknown as true },
  });
  ...
  const enviar = formulario.handleSubmit(async (dados) => {
    ...
    try {
      await apiDeSessao.cadastrar({
        ...
        email: dados.email,
        senha: dados.senha,
        plano: dados.plano as IdDoPlano,
      });
      await recarregar();
      navegar('/app?bem-vindo=1', { replace: true });
    } catch (causa) {
    ...
    >
      <form onSubmit={(e) => void enviar(e)} noValidate className="space-y-5">
        ...
        <Campo rotulo="Empresa ou escritório" erro={errors.nomeOrganizacao?.message}>
          <Entrada autoComplete="organization" placeholder="Aurora Tecnologia" icone={<Building2 />} {...formulario.register('nomeOrganizacao')} />
        </Campo>

        <div className="grid gap-5 sm:grid-cols-2">
          <Campo rotulo="Seu nome" erro={errors.nome?.message}>
            <Entrada autoComplete="name" placeholder="Ana Ribeiro" icone={<User />} {...formulario.register('nome')} />
          </Campo>
        ...
        <Campo rotulo="E-mail de trabalho" erro={errors.email?.message}>
          <Entrada type="email" autoComplete="email" placeholder="ana@empresa.com.br" icone={<Mail />} {...formulario.register('email')} />
        </Campo>

        <Campo rotulo="Senha" erro={errors.senha?.message}>
          <Entrada type="password" autoComplete="new-password" placeholder="Crie uma senha forte" icone={<Lock />} {...formulario.register('senha')} />
        </Campo>
        <ForcaDaSenha senha={senha} />

        <Campo rotulo="Confirme a senha" erro={errors.confirmacao?.message}>
          <Entrada type="password" autoComplete="new-password" placeholder="Repita a senha" icone={<Lock />} {...formulario.register('confirmacao')} />
        </Campo>

        <div>
          <label className="text-tinta-2 flex items-start gap-3 text-sm">
            <input type="checkbox" className="accent-brand-600 mt-0.5 size-4 rounded" {...formulario.register('termos')} />
            <span>
              Li e aceito os termos de uso e a política de privacidade. Entendo que as assinaturas coletadas têm
              validade jurídica nos termos da Lei 14.063/2020.
            </span>
          </label>
          {errors.termos && <p className="text-perigo-tinta mt-1.5 text-xs font-medium">{errors.termos.message}</p>}
        </div>
