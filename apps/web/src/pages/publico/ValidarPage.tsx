    ...
    <>
      ...
      <main id="conteudo">
        <section className="bg-noite-950 relative isolate overflow-hidden pt-32 pb-16 text-white">
          ...
          <div
            aria-hidden="true"
            className="absolute -top-40 left-1/2 -z-10 h-96 w-[60rem] -translate-x-1/2 rounded-full bg-[radial-gradient(closest-side,rgba(99,102,241,0.3),transparent)]"
          />
          <div className="mx-auto max-w-3xl px-5 text-center">
            ...
            {/* `key`: outro código na URL recomeça o campo com ele, sem efeito de sincronização. */}
            <Busca key={codigo ?? ''} codigoAtual={codigo} />
          </div>
        </section>

        <section className="mx-auto max-w-6xl px-5 py-12">
          {codigo ? <Resultado codigo={codigo} /> : <ComoFunciona />}
        </section>
      </main>
      <Rodape />
    </>
...
function Busca({ codigoAtual }: { readonly codigoAtual?: string }) {
  ...
  const buscar = (evento: FormEvent) => {
    ...
    if (limpo) void navegar(`/validar/${limpo}`);
  };
  ...
  const conferirArquivo = async (arquivo: File | undefined) => {
    ...
    try {
      ...
      void navegar(`/validar/${encontrado}?arquivo=${versao}&hash=${hash}`);
    } catch (causa) {
      definirErro(
        causa instanceof ErroDaApi && causa.status === 404 ? causa.message : mensagemDoErro(causa),
      );
    } finally {
  ...
  return (
    <div className="mx-auto mt-10 max-w-2xl space-y-4 text-left">
      <form
        onSubmit={buscar}
        className="flex gap-2 rounded-2xl border border-white/10 bg-white/5 p-2 backdrop-blur"
      >
        ...
        <Botao type="submit" variante="claro" tamanho="lg">
          Validar
        </Botao>
      </form>
      ...
      <label
        ...
        className={cn(
          ...
          arrastando
            ? 'border-ciano-300 bg-white/10 text-white'
      ...
      >
        <input
          ...
          className="sr-only"
          onChange={(e) => void conferirArquivo(e.target.files?.[0])}
        />
        {conferindo ? (
        ...
        <span>
          {conferindo
            ? 'Calculando o hash e conferindo…'
            : 'Ou arraste o PDF aqui — o hash é calculado no seu navegador, o arquivo não é enviado'}
        </span>
      </label>
...
function Resultado({ codigo }: { readonly codigo: string }) {
  ...
  const consulta = useQuery({
    queryKey: ['validacao', codigo],
    queryFn: () => apiDeValidacao.porCodigo(codigo),
    retry: false,
  });
...
type Veredito = {
  tom: 'sucesso' | 'alerta' | 'perigo' | 'neutro';
  titulo: string;
  texto: string;
  icone: ReactNode;
};
...
function veredito(dados: ResultadoDaValidacao): Veredito {
  const assinaturasOk = dados.signatarios.every(
    (s) => s.assinaturaDigital === null || s.assinaturaDigital.valida,
  );
  ...
  if (!trilha.integra || !assinaturasOk || (selo.presente && !selo.valido)) {
    return {
      tom: 'perigo',
      icone: <ShieldAlert />,
      titulo: 'Atenção: a integridade não confere',
      texto:
        'Algum registro deste documento não passou na verificação criptográfica. Não confie nele sem conferir com quem o enviou.',
    };
  }
  ...
  if (documento.status === 'em_andamento') {
    return {
      tom: 'alerta',
      icone: <Clock3 />,
      titulo: 'Assinaturas em andamento',
      texto:
        'O documento é autêntico, mas ainda não foi assinado por todos. As assinaturas já feitas são válidas.',
    };
  }
  ...
  return {
    tom: 'neutro',
    icone: <XCircle />,
    titulo: 'Documento encerrado sem conclusão',
    texto: 'Este documento foi cancelado, recusado ou expirou antes de todas as assinaturas.',
  };
}
...
function Laudo({
  ...
  readonly dados: ResultadoDaValidacao;
  readonly arquivo: 'original' | 'assinado' | null;
}) {
  ...
  return (
    <div className="space-y-6">
      <motion.div
        ...
        className={cn(
          'flex flex-col gap-5 rounded-[var(--radius-cartao)] bg-gradient-to-br p-6 text-white sm:flex-row sm:items-center sm:p-8',
          TONS_DO_VEREDITO[resultado.tom],
        )}
      >
        <span className="flex size-16 shrink-0 items-center justify-center rounded-2xl bg-white/15 [&_svg]:size-9">
          {resultado.icone}
        </span>
        ...
        <span className="rounded-xl bg-white/15 px-3 py-2 font-mono text-sm font-semibold">
          {documento.codigo}
        </span>
      </motion.div>
      ...
      {arquivo && (
        <Alerta tom="sucesso" titulo="O arquivo que você enviou confere">
          O hash SHA-256 do seu arquivo é idêntico ao do PDF{' '}
          {arquivo === 'assinado'
            ? 'final assinado emitido pela plataforma'
            : 'original enviado para assinatura'}
        ...
        <div className="space-y-6">
          <Cartao>
            <CabecalhoDoCartao
              ...
              descricao={`${documento.organizacao} · enviado por ${documento.remetente}`}
              acoes={<EtiquetaDoDocumento status={documento.status} />}
            />
          ...
          <Cartao>
            <CabecalhoDoCartao
              titulo={`Signatários (${dados.signatarios.length})`}
              descricao="Cada assinatura digital é verificada agora contra a chave pública da plataforma."
            />
                ...
                <li key={s.ordem} className="border-linha rounded-2xl border p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-tinta-2 mt-1 text-xs">
                        {s.assinadoEm
                          ? `Assinou em ${formatarDataHoraLonga(s.assinadoEm)}`
                          : s.recusadoEm
                            ? `Recusou em ${formatarDataHora(s.recusadoEm)}`
                            : 'Ainda não assinou'}
                      </p>
                    </div>
                    {s.assinaturaDigital &&
                  ...
                  {s.verificacoes.length > 0 && (
                    <ul
                      ...
                      aria-label="Identidade verificada por"
                    >
                      {s.verificacoes.map((v) => {
                        ...
                        return (
                          ...
                          >
                            <Icone className="text-sucesso size-3.5" aria-hidden="true" />{' '}
                            {VERIFICACAO[v.tipo].curto}
                            {v.pontuacao !== null && v.tipo !== 'codigo_email' && (
                              <span className="text-tinta-3 numeros">
                                {Math.round(v.pontuacao * 100)}%
                              </span>
                            )}
                          </li>
                        );
                  ...
                  {s.assinaturaDigital && (
                    <p className="text-tinta-3 mt-2 font-mono text-[11px]">
                      id da assinatura {s.assinaturaDigital.id}
                    </p>
                  )}
                </li>
              ))}
        ...
        <div className="space-y-6">
          <Cartao>
            ...
            <ul className="space-y-4">
              <Prova
                ok={selo.valido}
                pendente={!selo.presente}
                icone={<KeyRound />}
                titulo="Selo da plataforma"
                texto={
                  selo.presente
                    ? `${selo.algoritmo} · chave ${selo.idDaChave}`
                    : 'Emitido quando todos assinarem'
                }
              />
              <Prova
                ok={dados.signatarios.every(
                  (s) => s.assinaturaDigital === null || s.assinaturaDigital.valida,
                )}
                icone={<FileCheck2 />}
              ...
              <Prova
                ...
                texto={
                  trilha.integra
                    ? `${trilha.total} eventos encadeados, nenhum alterado`
                    : `Quebra no evento ${trilha.quebraEm}: ${trilha.motivo}`
                }
              />
            </ul>
            <div className="border-linha mt-5 space-y-3 border-t pt-5">
              <div>
                <p className="text-tinta-3 mb-1 flex items-center gap-1.5 text-xs font-semibold uppercase">
                  <Fingerprint className="size-3.5" /> SHA-256 do original
                </p>
              ...
              {documento.hashAssinado && (
                <div>
                  <p className="text-tinta-3 mb-1 flex items-center gap-1.5 text-xs font-semibold uppercase">
                    <ShieldCheck className="size-3.5" /> SHA-256 do PDF assinado
                  </p>
            ...
            <a
              href={apiDeValidacao.urlDaChave()}
              target="_blank"
              rel="noreferrer"
              className="text-destaque mt-5 inline-flex items-center gap-1.5 text-sm font-semibold hover:underline"
            >
          ...
          <Cartao>
            <CabecalhoDoCartao
              ...
              descricao="Eventos públicos da trilha de auditoria"
            />
                  ...
                  <span
                    ...
                    className={cn(
                      ...
                      e.acao === 'documento_concluido'
                        ? 'bg-sucesso'
                        : e.acao.includes('recusa') || e.acao.includes('cancel')
                          ? 'bg-perigo'
                          : 'bg-destaque',
                    )}
                  />
    ...
    <li className="flex items-start gap-3">
        ...
        className={cn(
          ...
          pendente
            ...
            : ok
              ...
              : 'bg-perigo-suave text-perigo-tinta',
        )}
      >
        {icone}
      </span>
      <div className="min-w-0 flex-1">
        <p className="text-tinta flex items-center gap-1.5 text-sm font-semibold">
          ...
          {!pendente &&
            (ok ? (
              <CheckCircle2 className="text-sucesso size-4" aria-label="válido" />
            ) : (
              <AlertTriangle className="text-perigo size-4" aria-label="inválido" />
            ))}
        </p>
  ...
  const itens = [
    {
      ...
      texto: 'Ele está no rodapé de todas as páginas do PDF assinado, no formato LS-XXXX-XXXX.',
    },
    {
      icone: UploadCloud,
      titulo: 'Ou arraste o PDF',
      texto:
        'Calculamos a impressão digital SHA-256 no seu navegador e procuramos o documento correspondente.',
    },
    {
      icone: ShieldCheck,
      titulo: 'Veja o laudo',
      texto:
        'Assinaturas Ed25519, selo da plataforma e trilha de auditoria são verificados na hora.',
    },
  ];
  ...
  return (
    <div className="grid gap-6 md:grid-cols-3">
      ...
      <p className="text-tinta-3 text-center text-sm md:col-span-3">
        ...
        para ver os documentos completos.
      </p>
    </div>
  );
}
