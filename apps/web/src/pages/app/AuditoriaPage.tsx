...
import { Bot, Download, FileText, Link2, Search, ShieldAlert, ShieldCheck, User, UserRoundPen } from 'lucide-react';
...
const ATORES = {
  ...
  signatario: { rotulo: 'Signatário', icone: UserRoundPen, classe: 'bg-sucesso-suave text-sucesso-tinta' },
  ...
  return (
    <>
      <CabecalhoDaPagina
        ...
        acoes={
          <>
            <Botao variante="secundario" icone={<ShieldCheck className="size-4" />} carregando={exame.isPending} onClick={() => exame.mutate()}>
      ...
      {exame.data &&
        (exame.data.integra ? (
          <Alerta tom="sucesso" titulo="Trilha íntegra" className="mb-6">
            {exame.data.eventos} eventos em {exame.data.cadeias} cadeias recalculados agora ({formatarDataHora(exame.data.verificadoEm)}). Nenhum registro foi alterado, apagado ou inserido fora de ordem.
          </Alerta>
        ) : (
          <Alerta tom="perigo" titulo={`${exame.data.quebradas.length} cadeia(s) com quebra`} className="mb-6">
      ...
      {exame.isError && <Alerta tom="perigo" className="mb-6">Não foi possível examinar a trilha agora.</Alerta>}
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row">
        ...
        <label htmlFor="filtro-acao" className="sr-only">Filtrar por ação</label>
        <Seletor
          id="filtro-acao"
          className="sm:w-72"
          value={acao}
        ...
        >
          ...
          {consulta.data?.acoes.map((a) => (
            <option key={a.acao} value={a.acao}>{rotuloDaAcao(a.acao)} ({a.total})</option>
          ))}
        </Seletor>
      </div>
        ...
        {consulta.isPending && (
          <Carregando>
            <div className="space-y-px">
              {Array.from({ length: 8 }, (_, i) => <Esqueleto key={i} className="h-16 rounded-none" />)}
            </div>
          </Carregando>
        )}
        {consulta.isError && <EstadoDeErro erro={consulta.error} aoTentarDeNovo={() => void consulta.refetch()} />}
        {consulta.data?.itens.length === 0 && <EstadoVazio icone={<Search />} titulo="Nenhum evento encontrado" descricao="Mude a busca ou o filtro de ação." />}
        {consulta.data && consulta.data.itens.length > 0 && (
          <ol className="divide-linha divide-y">
            {consulta.data.itens.map((evento) => (
              <Evento key={evento.id} evento={evento} aberto={aberto === evento.id} aoAlternar={() => definirAberto(aberto === evento.id ? null : evento.id)} />
            ))}
          </ol>
        )}
      </Cartao>

      {consulta.data && <Paginacao pagina={consulta.data.pagina} porPagina={consulta.data.porPagina} total={consulta.data.total} aoMudar={definirPagina} />}
    </>
  );
}

function Evento({ evento, aberto, aoAlternar }: { readonly evento: EventoDeAuditoria; readonly aberto: boolean; readonly aoAlternar: () => void }) {
  ...
  return (
    <li>
      <button type="button" onClick={aoAlternar} aria-expanded={aberto} className="hover:bg-superficie-2/60 flex w-full items-start gap-4 px-5 py-4 text-left transition">
        <span className={cn('mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-xl', falha ? 'bg-perigo-suave text-perigo-tinta' : ator.classe)}>
          {falha ? <ShieldAlert className="size-4" aria-hidden="true" /> : <Icone className="size-4" aria-hidden="true" />}
        </span>
        ...
        <span className="text-tinta-3 hidden shrink-0 font-mono text-xs sm:block">{evento.hash.slice(0, 10)}…</span>
      </button>
      ...
      {aberto && (
        <div className="bg-superficie-2/50 border-linha animate-surgir grid gap-3 border-t px-5 py-4 text-xs sm:grid-cols-2">
          ...
          <Dado rotulo="Ator" valor={`${ator.rotulo}${evento.atorNome ? ` · ${evento.atorNome}` : ''}`} />
          <Dado rotulo="Cadeia · sequência" valor={`${evento.cadeia} · #${evento.sequencia}`} mono />
          ...
          {evento.userAgent && <Dado rotulo="Navegador" valor={evento.userAgent} className="sm:col-span-2" />}
          {evento.documento?.uuid && (
            <Link to={`/app/documentos/${evento.documento.uuid}`} className="text-destaque inline-flex items-center gap-1.5 font-semibold hover:underline sm:col-span-2">
...
function Dado({ rotulo, valor, mono = false, className }: { readonly rotulo: string; readonly valor: string; readonly mono?: boolean; readonly className?: string }) {
  return (
