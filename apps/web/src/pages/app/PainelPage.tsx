            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-white/80">
                  Envie o primeiro PDF: em poucos minutos ele volta assinado, selado e com QR Code
                  de validação.
                </p>
              </div>
            </div>
...
function ConteudoDoPainel({
  ...
  readonly dados: NonNullable<Awaited<ReturnType<typeof apiDoPainel.resumo>>>;
}) {
    ...
    <div className="space-y-6">
      <div className="grid gap-4 *:min-w-0 sm:grid-cols-2 xl:grid-cols-4">
        <Indicador
          ...
          rotulo="Em andamento"
          valor={String(indicadores.emAndamento)}
          detalhe="aguardando assinaturas"
        />
        <Indicador
          indice={1}
          icone={<CheckCircle2 />}
          ...
          rotulo="Concluídos"
          valor={String(indicadores.concluidos)}
          detalhe="assinados por todos"
        />
        <Indicador
          indice={2}
          icone={<Gauge />}
          ...
          valor={formatarPorcentagem(indicadores.taxaDeConclusao)}
          detalhe="dos documentos enviados"
        />
        <Indicador
          indice={3}
          icone={<Timer />}
          ...
          rotulo="Tempo médio"
          valor={formatarDuracao(indicadores.tempoMedioDeConclusaoHoras)}
          detalhe="do envio à última assinatura"
        />
      </div>
      ...
      <div className="grid gap-6 *:min-w-0 xl:grid-cols-3">
        <Cartao className="xl:col-span-2">
          <CabecalhoDoCartao
            ...
            descricao="Documentos enviados e concluídos por mês"
          />
          <GraficoMensal
        ...
        <Cartao>
          ...
          <GraficoDeRosca
            ...
            fatias={STATUS_EM_ORDEM.map((s) => ({
              rotulo: STATUS_DO_DOCUMENTO[s].rotulo,
              valor: dados.porStatus[s] ?? 0,
              cor: COR_DO_STATUS[s],
            }))}
          />
          <ul className="mt-6 space-y-2">
            {STATUS_EM_ORDEM.filter((s) => (dados.porStatus[s] ?? 0) > 0).map((s) => (
              <li key={s} className="flex items-center justify-between text-sm">
                <Link
                  to={`/app/documentos?status=${s}`}
                  className="text-tinta-2 hover:text-tinta flex items-center gap-2.5"
                >
                  <span
      ...
      <div className="grid items-start gap-6 *:min-w-0 lg:grid-cols-2">
        <Cartao>
          <CabecalhoDoCartao
            ...
            descricao="Documentos em que é a sua vez"
          />
          {dados.aguardandoVoce.length === 0 ? (
            <EstadoVazio
              ...
              icone={<Inbox />}
              titulo="Tudo em dia"
              descricao="Nenhum documento esperando por você agora."
            />
          ) : (
            <ul className="divide-linha -mx-2 divide-y">
              {dados.aguardandoVoce.map((item) => (
                <li key={item.documento}>
                  <Link
                    to={`/app/documentos/${item.documento}`}
                    className="hover:bg-superficie-2 group flex items-center gap-4 rounded-xl px-2 py-3 transition"
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="text-tinta block truncate text-sm font-semibold">
                        {item.titulo}
                      </span>
                      <span className="text-tinta-3 block text-xs">
                        ...
                        {item.prazo
                          ...
                          : `enviado ${formatarRelativo(item.enviadoEm)}`}
                      </span>
                    </span>
                    <ArrowRight
                      className="text-tinta-3 group-hover:text-destaque size-4 transition group-hover:translate-x-0.5"
                      aria-hidden="true"
                    />
                  </Link>
                </li>
        ...
        <Cartao>
          <CabecalhoDoCartao
            ...
            descricao="Registrada na trilha de auditoria"
          />
          {dados.atividade.length === 0 ? (
            <EstadoVazio
              ...
              descricao="Os eventos dos documentos aparecem aqui."
            />
          ) : (
      ...
      <Cartao>
        <CabecalhoDoCartao
          titulo="Verificações de identidade aprovadas"
          descricao="Total acumulado, por método"
        />
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {(Object.keys(VERIFICACAO) as TipoDeVerificacao[]).map((tipo) => {
            ...
            return (
              <div
                key={tipo}
                className="border-linha bg-superficie-2/50 flex items-center gap-4 rounded-2xl border p-4"
              >
                <span className="bg-destaque-suave text-destaque flex size-11 items-center justify-center rounded-xl">
                  <Icone className="size-5" aria-hidden="true" />
                </span>
                <div>
                  <p className="font-display text-tinta text-2xl font-extrabold numeros">
                    {dados.verificacoes[tipo] ?? 0}
                  </p>
    ...
    >
      <Cartao className="group relative h-full overflow-hidden transition hover:-translate-y-0.5 hover:shadow-elevada">
        <div
          aria-hidden="true"
          className={cn(
            'absolute -top-10 -right-10 size-28 rounded-full bg-gradient-to-br opacity-10 transition group-hover:opacity-20',
            cor,
          )}
        />
        <span
          className={cn(
            'flex size-11 items-center justify-center rounded-xl bg-gradient-to-br text-white shadow-lg [&_svg]:size-5',
            cor,
          )}
          aria-hidden="true"
        >
