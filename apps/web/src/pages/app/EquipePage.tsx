...
export function EquipePage() {
  ...
  const atualizar = useMutation({
    mutationFn: ({ uuid, dados }: { uuid: string; dados: { papel?: Papel; ativo?: boolean } }) =>
      apiDaEquipe.atualizar(uuid, dados),
    onSuccess: async () => {
  ...
  const alternarAtivo = async (membro: Membro) => {
    const ok = await confirmar(
      membro.ativo
        ? {
            titulo: `Desativar ${membro.nome}?`,
            mensagem:
              'A pessoa perde o acesso imediatamente e todas as sessões abertas são encerradas. Os documentos dela continuam na organização.',
            confirmar: 'Desativar',
            perigosa: true,
          }
        : {
            titulo: `Reativar ${membro.nome}?`,
  ...
  return (
    <>
      <CabecalhoDaPagina
        ...
        acoes={
          podeGerenciar && (
            <Botao icone={<UserPlus className="size-4" />} onClick={() => definirConvidando(true)}>
              Convidar pessoa
            </Botao>
          )
        }
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-4">
        {(Object.keys(ROTULO_DO_PAPEL) as Papel[]).map((papel) => (
          <Cartao key={papel} className="p-4 sm:p-4">
            ...
            <p className="font-display text-tinta mt-1 text-2xl font-extrabold numeros">
              {consulta.data?.filter((m) => m.papel === papel && m.ativo).length ?? '—'}
            </p>
      ...
      <Cartao semPreenchimento>
        ...
        {consulta.data?.length === 0 && (
          <EstadoVazio
            ...
            descricao="Convide colegas para enviar e acompanhar documentos juntos."
          />
        )}
                  ...
                  <li
                    ...
                    className={
                      membro.ativo
                        ...
                        : 'flex flex-col gap-4 p-5 opacity-60 sm:flex-row sm:items-center'
                    }
                  >
                      ...
                      <div className="min-w-0">
                        <p className="text-tinta flex flex-wrap items-center gap-2 font-semibold">
                          ...
                          {membro.convitePendente && (
                            <Etiqueta tom="alerta" icone={<Clock3 />}>
                              Convite pendente
                            </Etiqueta>
                          )}
                          {!membro.ativo && (
                            <Etiqueta tom="neutro" icone={<UserX />}>
                              Desativado
                            </Etiqueta>
                          )}
                          {membro.mfaAtivo && (
                        ...
                        <p className="text-tinta-3 text-xs">
                          {membro.ultimoAcessoEm
                            ? `Último acesso ${formatarRelativo(membro.ultimoAcessoEm)}`
                            : 'Nunca entrou'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {podeGerenciar && !souEu ? (
                        <>
                          <label className="sr-only" htmlFor={`papel-${membro.uuid}`}>
                          ...
                          <Seletor
                            ...
                            disabled={
                              !membro.ativo ||
                              atualizar.isPending ||
                              (membro.papel === 'proprietario' && perfil.papel !== 'proprietario')
                            }
                            onChange={(e) =>
                              atualizar.mutate({
                                uuid: membro.uuid,
                                dados: { papel: e.target.value as Papel },
                              })
                            }
                          >
                          ...
                          {membro.convitePendente && membro.ativo && (
                            <Botao
                              variante="fantasma"
                              tamanho="sm"
                              icone={<MailPlus className="size-4" />}
                              onClick={() => reenviar.mutate(membro.uuid)}
                              aria-label={`Reenviar convite para ${membro.nome}`}
                            >
                          ...
                          <Botao
                            ...
                            aria-label={
                              membro.ativo ? `Desativar ${membro.nome}` : `Reativar ${membro.nome}`
                            }
                          >
      ...
      <ModalDeConvite
        aberto={convidando}
        aoFechar={() => definirConvidando(false)}
        podeProprietario={perfil.papel === 'proprietario'}
      />
    </>
  );
}
...
function ModalDeConvite({
  ...
  readonly podeProprietario: boolean;
}) {
  const clienteDeQuery = useQueryClient();
  ...
  const convidar = useMutation({
    mutationFn: () =>
      apiDaEquipe.convidar({ nome: nome.trim(), email: email.trim().toLowerCase(), papel }),
    ...
    <Modal
      ...
      rodape={
        <>
          ...
          <Botao
            disabled={nome.trim().length < 2 || !/^\S+@\S+\.\S+$/.test(email.trim())}
      ...
      <div className="space-y-4">
        ...
        <Campo rotulo="E-mail">
          <Entrada
            ...
            placeholder="eduarda@empresa.com.br"
          />
        </Campo>
