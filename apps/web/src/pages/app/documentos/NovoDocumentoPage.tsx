...
function tituloDoArquivo(nome: string): string {
  const base = nome.replace(/\.pdf$/i, '').replace(/[_-]+/g, ' ').trim();
...
export function NovoDocumentoPage() {
  ...
  const envio = useMutation({
    mutationFn: () => apiDeDocumentos.criar({ arquivo: arquivo as File, titulo: titulo.trim(), mensagem: mensagem.trim() || undefined }),
    onSuccess: async (documento) => {
      ...
      navegar(`/app/documentos/${documento.uuid}/preparar`, { replace: true });
    },
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        <Cartao>
          ...
          {arquivo === null ? (
            <label
              ...
              className={cn(
                ...
                arrastando ? 'border-destaque bg-destaque-suave' : 'border-linha-forte hover:border-destaque hover:bg-superficie-2',
              )}
            >
              <input type="file" accept="application/pdf,.pdf" className="sr-only" onChange={(e) => void escolher(e.target.files?.[0])} />
              ...
              <p className="text-tinta-2 mt-1 text-sm">
                ou <span className="text-destaque font-semibold">escolha no computador</span> · até 20 MB
              </p>
            </label>
          ) : (
            <div className="space-y-5">
              ...
              <div className="border-destaque/20 bg-destaque-suave/60 rounded-2xl border p-4">
                ...
                <p className="text-destaque-tinta mt-2 font-mono text-xs break-all">{hash ?? 'calculando…'}</p>
                <p className="text-tinta-2 mt-2 text-xs">Calculada no seu navegador. Se um único byte do arquivo mudar depois, este código muda — e a validação denuncia.</p>
              </div>

              <Campo rotulo="Título do documento" dica="É o que quem assina vê no e-mail e na tela de assinatura.">
                <Entrada value={titulo} maxLength={160} onChange={(e) => definirTitulo(e.target.value)} />
              </Campo>
              <Campo rotulo="Mensagem para quem assina" opcional>
                <AreaDeTexto value={mensagem} maxLength={1000} placeholder="Ex.: Olá! Segue o contrato revisado conforme combinamos." onChange={(e) => definirMensagem(e.target.value)} />
              </Campo>

              <div className="flex justify-end">
                <Botao tamanho="lg" carregando={envio.isPending} disabled={titulo.trim().length < 3} onClick={() => envio.mutate()}>
          ...
          <p className="text-tinta-3 mt-6 flex items-center gap-2 text-xs">
            ...
            O arquivo fica em armazenamento privado. Não existe link público para ele.
          </p>
        </Cartao>

        <div>{arquivo ? <VisualizadorDePdf arquivo={arquivo} alturaMaxima="70dvh" /> : <PrevisaoVazia />}</div>
      </div>
    </>
  );
}
...
function PrevisaoVazia() {
  return (
    <div className="border-linha text-tinta-3 flex h-full min-h-80 flex-col items-center justify-center rounded-2xl border border-dashed p-10 text-center text-sm">
      <FileText className="mb-3 size-10 opacity-40" aria-hidden="true" />A pré-visualização do PDF aparece aqui.
    </div>
  );
}
