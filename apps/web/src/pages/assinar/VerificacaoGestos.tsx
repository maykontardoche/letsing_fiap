...
const MODELO = 'https://storage.googleapis.com/mediapipe-models/gesture_recognizer/gesture_recognizer/float16/1/gesture_recognizer.task';
...
export function VerificacaoGestos({ api, aoAprovar }: { readonly api: ApiDeAssinatura; readonly aoAprovar: () => void }) {
  ...
  const comecar = async () => {
    ...
    try {
      const [desafio, visao] = await Promise.all([api.iniciarGestos(), import('@mediapipe/tasks-vision')]);
      const arquivos = await visao.FilesetResolver.forVisionTasks(`https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@${VERSAO_DO_MEDIAPIPE}/wasm`);
  ...
  const reconhecer = (
    reconhecedor: { recognizeForVideo: (video: HTMLVideoElement, instante: number) => { gestures: { categoryName: string; score: number }[][] } },
    pedida: Gesto[],
  ) =>
    new Promise<{ gestos: Gesto[]; confiancas: number[] } | null>((resolver) => {
      ...
      const passo = () => {
        ...
        if (video.currentTime !== ultimoQuadro) {
          ...
          const categoria = reconhecedor.recognizeForVideo(video, performance.now()).gestures[0]?.[0];
          const esperado = pedida[gestos.length] as Gesto;

          definirVisto(categoria && categoria.categoryName !== 'None' ? { gesto: categoria.categoryName, confianca: categoria.score } : null);
          ...
          if (seguidos.length >= QUADROS_PARA_ACEITAR) {
            ...
            confiancas.push(Math.round((seguidos.reduce((a, b) => a + b, 0) / seguidos.length) * 1000) / 1000);
            seguidos = [];
  ...
  return (
    <div className="space-y-5">
            ...
            return (
              <li
                ...
                className={cn(
                  ...
                  feito ? 'border-sucesso bg-sucesso-suave' : agora ? 'border-destaque bg-destaque-suave scale-105 shadow-brilho' : 'border-linha bg-superficie opacity-70',
                )}
              >
                <span className="text-tinta-3 absolute top-2 left-3 text-xs font-bold numeros">{i + 1}</span>
                {feito ? <CheckCircle2 className="text-sucesso-tinta size-9" aria-hidden="true" /> : <Icone className={cn('size-9', agora ? 'text-destaque' : 'text-tinta-2')} />}
      ...
      <div className="bg-noite-900 relative mx-auto aspect-[4/3] w-full max-w-lg overflow-hidden rounded-3xl">
        <video ref={camera.video} muted playsInline className={cn('size-full -scale-x-100 object-cover', fase === 'reconhecendo' ? 'opacity-100' : 'opacity-0')} />
          ...
          <>
            <div className="absolute inset-x-0 top-0 flex justify-between p-3 text-xs font-semibold text-white">
              <span className="rounded-full bg-black/50 px-2.5 py-1 backdrop-blur">
                {visto ? `Vendo: ${GESTOS[visto.gesto as Gesto]?.rotulo ?? visto.gesto} · ${Math.round(visto.confianca * 100)}%` : 'Mostre a mão inteira'}
              </span>
            </div>
            <div className="absolute inset-x-4 bottom-4 rounded-2xl bg-black/60 px-4 py-3 text-white backdrop-blur" aria-live="assertive">
              <p className="text-center text-base font-semibold">Faça: {GESTOS[atual].rotulo}</p>
              <p className="mt-0.5 text-center text-xs text-white/70">{GESTOS[atual].dica} — segure por um instante</p>
              <div className="mt-2.5 h-1.5 overflow-hidden rounded-full bg-white/15">
                <div className="h-full rounded-full bg-emerald-400 transition-all duration-150" style={{ width: `${segurando * 100}%` }} />
              </div>
            </div>
          </>
        )}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 p-6 text-center text-white">
            {fase === 'inicio' && (
              <>
                ...
                <p className="max-w-xs text-sm text-white/75">Vamos sortear três gestos. Faça cada um diante da câmera e segure até a barra encher.</p>
              </>
            )}
            {(fase === 'preparando' || fase === 'enviando') && (
              <>
                <Loader2 className="size-10 animate-spin text-brand-300" aria-hidden="true" />
                <p className="text-sm text-white/75">{fase === 'preparando' ? 'Carregando o reconhecimento de gestos…' : 'Registrando a verificação…'}</p>
              </>
            )}
            {fase === 'aprovada' && (
              <>
      ...
      {(fase === 'inicio' || fase === 'erro') && (
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-tinta-3 flex items-center gap-1.5 text-center text-xs">
            <ShieldCheck className="size-3.5" aria-hidden="true" /> O reconhecimento roda no seu dispositivo (MediaPipe). Nada da câmera é enviado.
          </p>
        </div>
      )}
    </div>
