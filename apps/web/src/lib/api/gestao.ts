...
export interface Painel {
  ...
  readonly serieMensal: readonly { readonly mes: string; readonly enviados: number; readonly concluidos: number }[];
  readonly verificacoes: Partial<Record<TipoDeVerificacao, number>>;
  readonly cota: { readonly usados: number; readonly limite: number | null; readonly plano: IdDoPlano };
  readonly aguardandoVoce: readonly { readonly documento: string; readonly titulo: string; readonly remetente: string; readonly prazo: string | null; readonly enviadoEm: string | null }[];
  readonly atividade: readonly { readonly acao: string; readonly resumo: string; readonly em: string; readonly documento: { readonly uuid?: string; readonly titulo?: string } | null }[];
}
...
export const apiDaEquipe = {
  ...
  convidar: (dados: { nome: string; email: string; papel: Papel }) => http.post<Membro>('/equipe', dados),
  atualizar: (uuid: string, dados: { papel?: Papel; ativo?: boolean }) => http.patch<Membro>(`/equipe/${uuid}`, dados),
...
export interface ExameDeIntegridade {
  ...
  readonly quebradas: readonly { readonly cadeia: string; readonly quebraEm: number | null; readonly motivo: string | null }[];
}
...
export const apiDeAuditoria = {
  listar: (filtro: { acao?: string; q?: string; pagina?: number }) => {
    ...
    return http.get<{ total: number; pagina: number; porPagina: number; acoes: { acao: string; total: number }[]; itens: EventoDeAuditoria[] }>(
...
export interface DadosDaOrganizacao {
  ...
  readonly criadoEm: string;
  readonly uso: { readonly enviadosNoMes: number; readonly limiteDeEnvios: number | null; readonly membros: number; readonly limiteDeMembros: number | null };
}
...
export interface Notificacoes {
  ...
  readonly itens: readonly { readonly id: number; readonly titulo: string; readonly mensagem: string; readonly link: string | null; readonly lida: boolean; readonly criadaEm: string }[];
}

export const apiDaConta = {
  organizacao: () => http.get<DadosDaOrganizacao>('/organizacao'),
  atualizarOrganizacao: (dados: { nome?: string; plano?: IdDoPlano }) => http.patch<DadosDaOrganizacao>('/organizacao', dados),
  atualizarPerfil: (nome: string) => http.patch<void>('/me', { nome }),
  trocarSenha: (dados: { senhaAtual: string; novaSenha: string }) => http.post<void>('/me/senha', dados),
  iniciarMfa: () => http.post<{ qrCode: string; segredo: string }>('/me/mfa/iniciar'),
  confirmarMfa: (codigo: string) => http.post<{ codigosDeRecuperacao: string[] }>('/me/mfa/confirmar', { codigo }),
