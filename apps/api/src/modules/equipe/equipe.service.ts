...
import type { Papel, Prisma, Usuario } from '@prisma/client';
...
export class EquipeService {
  ...
  async convidar(
    ...
    const organizacao = await this.prisma.db.organizacao.findUniqueOrThrow({
      where: { id: quem.organizacaoId },
    });
    const limite = LIMITES_DO_PLANO[organizacao.plano].membros;

    if (
      limite !== null &&
      (await this.prisma.db.usuario.count({ where: { ativo: true } })) >= limite
    ) {
      throw new UnprocessableEntityException(
        `O plano ${ROTULO_DO_PLANO[organizacao.plano]} permite até ${limite} pessoas na equipe.`,
      );
    }
    ...
    const existente = await semEscopoDeOrganizacao(() =>
      this.prisma.db.usuario.findUnique({ where: { email: dados.email }, select: { id: true } }),
    );

    if (existente !== null)
      throw new ConflictException('Este e-mail já tem uma conta no LetsSign.');

    const { token, hash } = gerarToken();
    const criado = await this.prisma.db.$transaction(async (tx) => {
      const usuario = await tx.usuario.create({
        data: comOrganizacaoDoContexto<Prisma.UsuarioUncheckedCreateInput>({
          nome: dados.nome,
          email: dados.email,
          papel: dados.papel,
        }),
  ...
  async atualizar(
    quem: UsuarioAutenticado,
    uuid: string,
    dados: { papel?: Papel; ativo?: boolean },
    origem: Origem,
  ) {
    ...
    if (alvo.id === quem.id)
      throw new BadRequestException('Você não pode alterar o próprio papel nem se desativar.');
    ...
    const deixaDeSerProprietarioAtivo =
      ...
      alvo.ativo &&
      ((dados.papel !== undefined && dados.papel !== 'proprietario') || dados.ativo === false);

    if (
      deixaDeSerProprietarioAtivo &&
      (await this.prisma.db.usuario.count({ where: { papel: 'proprietario', ativo: true } })) <= 1
    ) {
      throw new UnprocessableEntityException(
        'A organização precisa de pelo menos um proprietário ativo.',
      );
    }

    const atualizado = await this.prisma.db.usuario.update({
      where: { id: alvo.id },
      data: { papel: dados.papel, ativo: dados.ativo },
    });
  ...
  async reenviarConvite(quem: UsuarioAutenticado, uuid: string): Promise<void> {
    ...
    if (alvo === null) throw new NotFoundException('Membro não encontrado.');
    if (alvo.senhaHash !== null)
      throw new UnprocessableEntityException('Esta pessoa já aceitou o convite.');
...
function descrever(
  ...
  if (dados.ativo === false)
    return `${quem.nome} desativou ${alvo.nome}; as sessões dele foram encerradas.`;
