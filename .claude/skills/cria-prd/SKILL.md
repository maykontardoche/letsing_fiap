...
2. Derive the slug in kebab-case for the output directory: `tasks/prd-[feature-slug]/` (at the repository root).
3. Check `docs/status.md` and `docs/backlog/mvp.md` to place the feature in its backlog epic, when they exist.
...
2. Cover all areas from the clarification checklist:
   ...
   - **Users and Stories**: Primary users (which organization roles — `proprietario`, `administrador`, `membro`, `auditor` — and whether external signers or the public validation page are involved), user stories, main flows.
   - **Core Functionality**: Data inputs/outputs, actions.
   - **Scope and Planning**: What is NOT included, dependencies.
   - **Design and Experience**: UI/UX guidelines and accessibility (WCAG 2.2 AA, light and dark themes).
   - **Privacy and Evidence**: Personal or biometric data involved (LGPD), and which actions must appear in the audit trail.
...
1. Create a development plan including:
   ...
   - Areas requiring research (use Web Search for business rules — e.g. electronic signature regulation in Brazil, LGPD).
