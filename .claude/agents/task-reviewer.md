...
Leia `.claude/stack-profile.md` — em especial a seção da app onde a task foi escrita (`apps/api` ou `apps/web`) e a lista **Forbidden** — antes de revisar.

## Idioma

Escreva o artefato de review em Português (Brasileiro). Exemplos de código seguem as convenções do projeto (identificadores de domínio em português).
